...
would normally go inside the ggplot call. In **Biogrowth** a modified version of these arguments can be set by providing additional arguments to the **biogrowth** function as shown in this vignette. 
...
The prediction of times to reach a specified count is shown in a histogram. We can adjust the binwidth of the histogram by setting the `bin_width` argument. In this example values between 0.5 and 3 would be usefull.
...
**Cowplot** has a nice feature for combining plots into grids, we will use a combination of the previous examples here.  

#COMMENT, this might not be the best example as the figures are not that related. Should I make a set of related figures here?
...
Or, by making a single tibble from the information stored in the static_prediction objects we can make a combined plot, clearly showing the differences between  simulations  
