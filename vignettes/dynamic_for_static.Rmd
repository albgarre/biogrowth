---
title: "Using models with secondary models to predict growth under constant environmental conditions"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using models with secondary models to predict growth under constant environmental conditions}
...
The argument `environment` within the `predict_growth()` function can take two values: "constant" or "dynamic". In the first case, the function only takes a `primary_model` (indeed, passing a `secondary_model` will return a warning). The logic for this is that the secondary model describes how the environmental conditions affect the kinetic parameters of the primary model (the growth rate). Therefore, the minimum model to describe growth under static conditions is the one that only includes a primary model.

However, the function can still predict microbial growth under isothermal conditions using `environment="dynamic"`. This can be useful in situations were the environmental conditions are constant, but the response of the population is described using secondary models. 
...
the predicted population size is identical to the one calculated using `predict_growth` with `environment="constant"`
for the Baranyi model when $\mu = \mu_{opt}$ and $\lambda = \frac{ \ln \left(1 +1/Q_0 \right) }{\mu_{opt}}$.

```{r}
lambda <- Q0_to_lambda(q0, mu_opt, logbase_mu = exp(1))
...
The advantages of using a model including a secondary model for modelling growth under
