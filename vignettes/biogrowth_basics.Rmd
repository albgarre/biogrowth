...
entirely understood. Hence, usually population growth is analyzed using mathematical
...
The user can choose among a variety of models for the calculations (see below). 
...
describe the variation of the variable(s) of interest (usually the size of the 
microbial population) against time. These models are largely
empirical, so they have model parameters that depend on the environmental conditions. 
A typical example is the relationship between the specific growth rate and the storage 
temperature. Predictive microbiology uses so called, *secondary models* to describe 
the relationship between the parameters of the primary model and the environmental 
conditions.
...
where $N_0$ is the initial population size, $\mu$ is the maximum specific growth rate, $\lambda$ is the duration of the lag phase and $C$ is the difference between the initial population size and the maximum population size.
...
$$
The exponential phase is described considering that during this phase, the specific growth rate is constant, with slop $\mu_{max}$.
...
where $t_{max}$, defined is the time required for the population size to reach $N_{max}$.
...
The following table shows the unit system used in **biogrowth** for each quantity.
...
The specific growth rate can be interpreted as the slope of the exponential phase of the growth 
curve under static conditions. In **biogrowth**, calculations are done in log10 scale. 
Hence, the growth rate is also described in log10(units)/[time].


## Numerical methods

The **biogrowth** package uses **FME** for model fitting and **deSolve** for estimating
population growth under dynamic conditions. Namely, the differential equations required for dynamic calculations are solved using the `ode()` function from **deSolve**, which implements several numerical methods. Model fitting is done using the `modFit()` function from **FME** for linear regression and the `modMCMC()` function from the same package for MCMC fitting. The functions in **biogrowth** use the default settings of these functions, although this can changed using the `...` argument.
...
* `check`: boolean specifying whether to make some validity checks of model parametes (`TRUE` by default).
...
This model has 4 model parameters: `mu`, `lambda`, `C` and `logN0` (retrieved using `primary_model_data("modGompertz)`). All this information must be defined in a list:
...
The function uses gpplot with theme_cowplot(). Therefore, it can be edited using layers as usual in the ggplot2 package. For instance,
...
The function includes additional arguments to edit the aesthetics of the plot.
...
The **biogrowth** package can also be used for simulating growth under dynamic environmental conditions using the `predict_dynamic_growth()` function. For this, this function combines primary and secondary growth models. It has 6 arguments:
...
In our simulation we will consider two environmental factors: temperature and pH. We can define their variation using this tibble:
...
Next, we will define a CPM model for the effect of pH. Note that the model selection is for illustration purposes, not based on any scientific knowledge. First of all, we need to set the item `model` to `"CPM"`. Then, we need to define the model parametes (note this model also needs `xmax`).
...
To make life easier, we can visualize the simulation using the S3 method for plot:
...
The function includes several arguments to edit the aesthetics of the plot. 

```{r}
plot(dynamic_prediction, add_factor = "temperature", ylims= c(0, 7), 
     label_y1 = "Microbial count (log CFU/ml)", label_y2 = "Storage temperature (ºC)",
     line_col = "lightgreen", line_size = 2, line_type2 = 1)
```
...
The function `fit_isothermal_growth()` can be used to estimate the parameters of primary growth models from data obtained under static conditions. This function has 4 arguments:
...
The data used for model fitting is defined using the `fit_data` argument. It must be a tibble with two columns named `time` (the elapsed time) and `logN` (the decimal logarithm of the population size). 
...
This function uses non-linear regression (through the `modFit()` function of the **FME** package), so it requires initial values for every model parameter to fit. In the case of the Baranyi model, the model parameters are: `logN0`, `mu`, `lambda` and `logNmax`. The `fit_isothermal_growth()` function enables to fix any model parameter before model fit using the `known_pars` argument. For instance, we can fix the specific growth rate to 0.2 (no particular reason for this, just as a demonstration)
...
It also includes methods for `residuals`, `coef`, `vcov`, `deviance`, `fitted` and `predict`. The `plot()` method compares the data and the fitted curve
...
This plot can be edited passing additional arguments to `plot`.
...
The function `fit_secondary_growth()` can be used to estimate the values of the cardinal parameters from the specific growth rates observed in a series of growth experiments under static conditions. This function has 6 arguments:
...
The `fit_data` argument must be a tibble containing the specific growth rates observed in several experiment under static environmental conditions. It must have one column named `mu` with the observed growth rate. Then, it must have as many additional columns as environmental factors included in the experiment. 
...
Before doing the calculations, this function does several validity checks of the model parameters. If the fitting was succesful, it returns an instance of `FitSecondaryGrowth` with 5 items:
...
The tibble passed to the argument `fit_data` must have two columns named `time` (elapsed time) and `logN` (log population size), in the same format as `example_dynamic_growth`. 
...
The function does some checks of the validity of the model parameters (can be turned off using `check=FALSE`). If the fitting was successful, it returns an instance of `FitDynamicGrowth` with 6 items:
...
Each experiment is described using a list with two elements: `data` and `conditions`. The element `data` is a tibble with two columns (`time` and `logN`) that describe the decimal logarithm of the observed population size. 
...
This function does several validity checks of the model parameters (can be turned off passing `check=FALSE`). If the fit was successful, it returns an instance of `FitMultipleDynamicGrowth` with several elements. 
...
The function includes additional arguments to edit several aesthetics of the plot
...
Due to the relevance of uncertainty and variability, Quantitative Microbial Risk Assessment usually requires the calculation of stochastic simulations. The function `predict_stochastic_growth` enables to include parameter uncertainty on the model predictions. It has 5 arguments:
...
For this example, we will use the trilinear growth model
...
scale ("original"). Then, the function converts the parameter sample to the original scale 
...
As well as for `fit_dynamic_growth()`, the environmental conditions can be added to the plot using the `add_factor` argument. Other aesthetics can be edited passing additional arguments to `plot`.
...
Any environmental factor can be included in the plot using the `add_factor` argument. Also, the aesthetics of the plot can be edited passing additional arguments to `plot`.

```{r, fig.width = 7, fig.height=5}
plot(global_MCMC, add_factor = "temperature",
     line_col2 = "blue", line_size2 = .5, line_type2 = 2)
```
...
For shelf-life estimation and risk assessment, the storage time required to reach a critical population size is usually of interest. For stochastic simulations, this time is not defined by a single value but by a probability distribution that can be estimated using the `distribution_to_logcount()` function. This function has two arguments:
