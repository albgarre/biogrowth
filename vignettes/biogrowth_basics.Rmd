...
#COMMENT These three definitions can improve from a bit of layman perspective:
...
Lag phase  
Until the time elapsed exceeds the lag time $\lambda$, there is no growth.
$$
\log N(t) = \log N_0; t \leq \lambda
$$
Exponential phase  
As long as time is between $\lambda$ and $t_{max}$, growth is maximal
$$
\log N(t) = \log N_0 + \mu(t-\lambda); t\in(\lambda,t_{max})
$$

Stationary phase  
If time exceeds $t_{max}$, growth stops
$$
...
#COMMENT the original paper just mentions n=2 for Temp right? Should we mention the default here? 
...
$$
In this equation, $X_{min}$, $X_{opt}$ and $X_{max}$ are the minimum, maximum and optimum values of $X$ for microbial growth (usually called *cardinal parameters*). The coefficient $n$ is the shape parameter of the cardinal model.
...
The **biogrowth** package includes three datasets to aid in the understanding of its functions:
...
#COMMENT this is a bit confusing to me, as normally plot() refers to base R plotting functions (but I will adapt :P)
```{r}
...
#COMMENT can you give a sort of range for this theoretical substance? Or should the user play around a bit to find a value that suits the needs? 
```{r}
...
#COMMENT this throws a warning: collapsing to unique 'x' values, is the solver acting up?
```{r}
...
my_data <- tibble(time = c(0, 25, 50, 75, 100),
                  logN = c(2, 2.5, 7, 8, 8))
```
#COMMENT:for the beginners: can we add a couple of lines that show how to load an Excel file here? Or just one or two lines that mentions that this is also possible with Excel data?
#COMMENT It is dead obvious, but this package will attract N00B (PhD)students that have limited experience in R

#COMMENT Many people have more than one dataset to consider, we could even make an example of mapping the function over a list of tibbles? 
#COMMENT I have seen beginners copy-pasting the same code 8 times to make 8 fits... 
...
#COMMENT, I am not really sure why you would use a diferent model for pH, but that may be my lack of understanding of these models
#COMMENT but the difference between Zwietering and CPM is the interaction between terms right? Then why would you use different models for each parameter? 
```{r}
...
#COMMENT: I am having a very hard time so see what is different in these graphs, after adjusting the correlation matrix. Maybe a more extreme example? Or lower the boundaries on the uncertainty?  
...
Then, we can call the `fit_MCMC_growth()` using these arguments plus the argument `niter` that we will set to 500.
...
my_MCMC_fit <- fit_MCMC_growth(example_dynamic_growth, example_env_conditions, 
                                ...
                                sec_model_names, 
                                niter = 500,
                                updatecov = 10) #COMMENT probably a hacky solution, we need to look at what causes the low nr of accepted runs

```
#COMMENT I keep getting ~10% accepted runs, is this bc of the input settings? 
#COMMENT Users get stressed by this kind of warning 
...
# Error: Can't combine `..1` <deSolve> and `..2` <deSolve>. x Some attributes are incompatible. ℹ The author of the class should implement vctrs methods. ℹ See <https://vctrs.r-lib.org/reference/faq-error-incompatible-attributes.html>
...
#COMMENT: Gives warning:valuescollapsing to unique 'x'
```{r}
