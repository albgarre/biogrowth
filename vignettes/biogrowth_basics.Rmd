...
Several functions in the package make use of stochastic algorithm. For reproducibility, we will set the internal seed of R's internal random number generator (PRNG) to some arbitrary value

```{r}
set.seed(1241)
```

According to the usual conventions in predictive microbiology, in this document we will use $log$ to refer to the decimal logarithm and $ln$ for the natural logarithm.
...
where $X_min$ is the minimum value of $X$ enabling growth, $X_{opt}$ is the optimum value of $X$ for growth and $n$ is the order of the model. # How do I know the order of the model? Can you explain this in a bit more detail? 
...
$$
In this equation, $X_{min}$, $X_{opt}$ and $X_{max}$ are the minimum, maximum and optimum values of $X$ for microbial growth (usually called *cardinal parameters*). The coefficient $n$ is the order of the cardinal model. # Maybe shape parameter for $n$ is more intuitive for the less math savvy readers. Maybe with examples of common values? 
...
The plot is generated as a gpplot object. Therefore, it can be edited using layers as usual in the ggplot2 package. For instance,
...
```{r}
# again here, how do I know the order of the model?
...
```{r}
# how do I know that I need to use a CPM model here instead of a Zwietering? Or is this assumed knowledge for end users?
...
```{r}
# Am I missing the transformation argument? 
# If I run this I get a very long list of warnings: 
# New names:
# * NA -> ...1
# * NA -> ...2

# repeats a lot of times.. 
# The summary does work though 
...
* `secondary_mode` parameters of the secondary model. # Not  `secondary_model` ??
...
```{r}
# I also get a warning here: Don't know how to automatically pick scale for object of type deSolve/matrix. Defaulting to continuous.
plot(my_dyna_fit)
```
...
```{r}
# Double y-axis... Don't show this to Hadley =)  
...
stoc_growth <- predict_stochastic_growth(my_model, my_times, n_sims,
  mean_logN0 = 0.5, sd_logN0 = .2, # Is there a reason why you would start with 0 cells?
...
```{r}
# It is not obvious to me (visually) what the changes are..
...
```{r}
# It only accepted 8% - 12% of the MCMC runs, is that normal? Should we change the start parameters?
...
# throws a hughe error: collapsing to unique 'x' 500 x
# Is does seem to work tough, it gives a result in the later examples
