...
The `predict_growth()` function has 9 arguments, as well as an additional `...` argument to pass additional arguments to the functions used for making the predictions. The prediction is defined by the following 6 arguments:
...
Therefore, the Baranyi model requires the definition of the logarithm of the initial population size (`logN0`), the maximum population size in the stationary phase (`logNmax`), the growth rate during the exponential growth phase `mu` and the duration of the lag phase `lambda`. In this example, we will assign `logN0=1`, `logNmax=7`, `mu=0.2` and `lambda=20`. 
...
In the case of model predictions under constant environmental conditions, the simulations are calculated by solving the algebraic form of the model equation. The time points were it is calculated must be defined using the `times` argument, which is a numeric vector of any length. In this case, we will use a regular vector from 0 to 100 with 1,000 uniformly spaced points.
...
For a detailed description of the arguments admitted by the function, please check its help page (e.g. with `?plot.GrowthPrediction`). Note that the function returns an instance of `ggplot`, so it can be edited using additional layers
...
The function `predict_growth()` can account for the effect of changes in the environmental conditions on parameter $\mu$. This behaviour is triggered setting `environment="dynamic"`. This requires the definition of additional arguments:
...
In this case, we need the minimum (`xmin`) and optimum (`xopt`) cardinal values, as well as the order of the model (`n`). We define them using individual entries in the list:
...
As well as for predictions under constant environmental conditions, `time_to_size` can estimate the time required to reach a given population size:
